...
/**
 * Same-origin fallback for an executive's broadcast attachment, for when the
 * browser can't reach file storage directly (see api/enrollment/upload).
 */
export async function POST(request: NextRequest) {
  const admin = await getCurrentAdmin();
  if (!admin) {
    return NextResponse.json(
      { ok: false, error: "Your admin session has expired. Sign in again, then attach the file again." },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    console.error("[admin-attachment-fallback] could not read the request body", err);
    return NextResponse.json(
  ...
  try {
    ...
    console.error("[admin-attachment-fallback] storing the file failed", err);
    return NextResponse.json(
