 ...
 * for.
 */
export default async function AlumniCvPaymentReturnPage({
  searchParams,
}: {
    ...
    <>
      ...
      <section className="bg-white rounded-xl border border-line shadow-card p-5 sm:p-6 max-w-2xl">
        ...
        <p className="text-ink mt-2">
          {settled
            ? "You can download your CV as often as you like for the next year, including every time you change it."
            : outcome.ok
              ? "Paystack reported the payment as unsuccessful, so nothing has been charged and the CV is still locked. You can try again."
              : outcome.error}
        </p>
        <p className="mt-5">
          <Link href="/alumni/cv" className={buttonClasses("primary", "md")}>
            Back to my CV
          </Link>
        </p>
      </section>
