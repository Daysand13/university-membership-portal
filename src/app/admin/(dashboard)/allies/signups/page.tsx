...
export default async function AllySignupsPage({ searchParams }: { searchParams: Promise<{ show?: string }> }) {
  ...
  const columns: Column<(typeof signups)[number]>[] = [
    {
      header: "Person",
      cell: (signup) => (
        <>
          ...
          {signup.organization && <span className="block text-xs text-slate">{signup.organization}</span>}
        </>
      ),
    },
    { header: "Joining as", cell: (signup) => (signup.type === "CORPORATE" ? "Organisation" : "Individual") },
    {
      header: "Email",
      cell: (signup) => {
        const email = emailState(signup);
        return (
          <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${email.tone}`}>
            {email.label}
          </span>
        );
      },
    ...
    { header: "Joined", cell: (signup) => dateFormat.format(signup.createdAt) },
    {
      header: "Actions",
      actions: true,
      cell: (signup) => {
        ...
        return (
          <span className="inline-flex flex-wrap justify-end gap-2">
            {canList && (
              ...
              >
                List publicly
                <span className="sr-only"> — {signup.fullName}</span>
              </Link>
            )}
            {signup.status === "NEW" && (
  ...
  return (
    <div>
      ...
      <nav aria-label="Filter sign-ups" className="flex flex-wrap gap-1.5 mb-5">
        {[
      ...
      {signups.length === 0 ? (
        ...
        <DataTable caption="Ally sign-ups" rows={signups} rowKey={(signup) => signup.id} columns={columns} />
      )}
    </div>
  );
}
