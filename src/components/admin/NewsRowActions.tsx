...
import { setNewsStatusAction, deleteNewsAction } from "@/lib/actions/news-actions";
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      <button
        ...
        disabled={isPending}
        onClick={() =>
          startTransition(() => setNewsStatusAction(id, isPublished ? ContentStatus.DRAFT : ContentStatus.PUBLISHED))
        }
      ...
      <Link href={`/admin/news/${id}`} title="Edit" className="p-2 rounded-md text-slate hover:bg-surface-muted hover:text-primary-800">
        <Pencil size={15} />
      </Link>
      <ConfirmButton
        action={() => deleteNewsAction(id)}
        confirmMessage="Delete this article permanently? This can't be undone."
