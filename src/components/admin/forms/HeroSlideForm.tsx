...
export function HeroSlideForm({ slide }: { slide?: HeroSlide }) {
  ...
  return (
    <>
      <form ref={formRef} key={formKey} action={formAction} className="grid sm:grid-cols-2 gap-4 items-start">
      ...
      {!slide && (
        <SuccessDialog
          ...
          isPending={isPending}
          title="Slide added"
          description="It is in the homepage banner while it is switched on."
          againLabel="Add another slide"
          onAgain={resetForm}
          listHref="/admin/hero-slides"
          listLabel="Back to slides"
        />
      )}
    </>
  );
