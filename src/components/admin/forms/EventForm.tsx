...
export function EventForm({ event, categories }: { event?: Event; categories: EventCategory[] }) {
  ...
  return (
    <>
      <form ref={formRef} key={formKey} action={formAction} className="space-y-6">
      ...
      {!event && (
        <SuccessDialog
          ...
          isPending={isPending}
          title="Event created"
          description="It appears on the website once its status is Published."
          againLabel="Add another event"
          onAgain={resetForm}
          listHref="/admin/events"
          listLabel="Back to events"
        />
      )}
    </>
  );
